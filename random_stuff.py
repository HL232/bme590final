...
resp = requests.post(
    ...
    json={"email": email})
blurred_image_0 = byte_2_json(resp)
# attempt to confirm
resp = requests.post("http://127.0.0.1:5000/api/process/confirm", json=blurred_image_0)
content_0 = byte_2_json(resp)
# view_image(b64str_to_numpy(content_1["image_data"]))

# blur
resp = requests.post(
    "http://127.0.0.1:5000/api/process/blur",
    json={"email": email})
blurred_image = byte_2_json(resp)
# attempt to confirm
resp = requests.post("http://127.0.0.1:5000/api/process/confirm", json=blurred_image)
content_1 = byte_2_json(resp)
# view_image(b64str_to_numpy(content_1["image_data"]))
...
resp = requests.post(
    ...
    json={"email": email})
contrast_image = byte_2_json(resp)
# attempt to confirm
resp = requests.post("http://127.0.0.1:5000/api/process/confirm", json=contrast_image)
content_2 = byte_2_json(resp)
# view_image(b64str_to_numpy(content_2["image_data"]))

resp = requests.get(
    "http://127.0.0.1:5000/api/image/"
    "get_previous_image/{}".format(email))
content = byte_2_json(resp)
print(blurred_image_0["image_id"], content_0["image_id"],
      blurred_image["image_id"], content_1["image_id"],
      contrast_image["image_id"], content_2["image_id"],
      content["image_id"])
